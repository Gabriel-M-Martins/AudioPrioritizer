...
namespace VideoPlayerController
{
    ...
    {
        static List<int> banishedPids { get; set; }

        static void Main()
        {
            BanPids();

            Console.WriteLine("Opções:\n 1: Priorizar áudio.\n 2: Despriorizar áudio.");
            Console.WriteLine("\nEscolha uma opção: ");
            ...
            {
                Console.Clear();
                Console.WriteLine("Digita direito, mangolão.");
            }
        }

        static void BanPids()
        {
            banishedPids = new List<int>();
            Process[] shellxp = Process.GetProcessesByName("ShellExperienceHost");
            Process[] idle = Process.GetProcessesByName("Idle");
            Process[] combined = shellxp.Concat(idle).ToArray();

            foreach (var p in combined)
            {
                banishedPids.Add(p.Id);
            }
        }

        static void UnprioritizeAudio()
        {
            Console.Clear();

            List<int> pids = GetRelevantPids();
            foreach (int i in pids)
            {
                SetApplicationVolume(i, 100f);
            }
            Console.WriteLine("Todos os áudios retornaram a 100%.");
        }

        static void PrioritizeAudio()
        {
            Console.Clear();

            List<int> pids = GetRelevantPids();
            Console.WriteLine("Processos que estão a executar áudio:\n");
            foreach (int i in pids)
            {
                if (!banishedPids.Contains(i))
                {
                    Console.WriteLine("id: " + i + " | processo: " + Process.GetProcessById(i).ProcessName);
                }
            }
            Console.WriteLine("\nEscolha um processo (pelo id) para priorizar acima dos demais:");
            string chosenProcessString = Console.ReadLine();
            int chosenProcess;

            if (Int32.TryParse(chosenProcessString, out chosenProcess))
            {
                Console.WriteLine("\nEscolha um volume para os outros processos:");
                string chosenVolumeString = Console.ReadLine();
                int chosenVolume;

                if (Int32.TryParse(chosenVolumeString, out chosenVolume))
                {
                    List<int> x = pids;
                    x.Remove(chosenProcess);

                    float volumeFloat = Convert.ToSingle(chosenVolume);

                    foreach (var i in x)
                    {
                        SetApplicationVolume(i, volumeFloat);
                    }

                    Console.WriteLine($"Áudio do processo priorizado e todos os outros definidos para {volumeFloat}%.");
                }
                else
                {
                    Console.Clear();
        ...
        static List<int> GetRelevantPids()
        {
            List<int> pids = GetAllPids();
            List<int> toBeRemoved = new List<int>();
            foreach(int pid in pids)
            {
                try
                {
                    var tmpAppVol = GetApplicationVolume(pid).Value;
                }
                catch(Exception)
                {
                    toBeRemoved.Add(pid);
        ...
        {
            Process[] processesCollection = Process.GetProcesses();
            List<int> result = new List<int>();

            foreach(var processo in processesCollection)
            {
                result.Add(processo.Id);
            }
